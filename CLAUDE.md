...
- **FW-16** (automated review vs human PR review): not a choice to offer — implementation work defaults to the automated review policy, currently the interim evidence-led review (see [Reviewing implemented work](#reviewing-implemented-work--interim-evidence-led-review-change-0)).
...
### Reviewing implemented work — interim evidence-led review (Change 0)

**TEMPORARY OPERATING POLICY** — activated from `docs/prds/evidence-led-review-lifecycle.md` (Change 0), in force until the evidence-led review lifecycle ships. It SUPERSEDES the former #301/FG-436 repeated-loop policy: no `--max-rounds 2` default, no rerunning the loop until the reviewer passes, no fixer-per-round remediation, and a reviewer pass is never, by itself, evidence that earlier findings were resolved. This changes how model review is CONDUCTED — every deterministic and authority gate is preserved unchanged (rule 11 below). It is instruction-only: no ledger tables, coordinator, or new gates exist yet (those are Changes 1–3).

Once an implementation's **initial commit/range has landed**, review it in this order:

1. **Deterministic verification BEFORE model review.** Confirm the change's deterministic gates are green at the exact sha — the required CI checks, or the documented evidence-reuse/local-fallback path — before dispatching any reviewer. No model verdict substitutes for a red deterministic gate.
2. **ONE risk-targeted discovery pass — not repeated open-ended sampling.** Target the pass at the change's actual risk surface (the invariants, trust boundaries, and failure modes the diff can plausibly reach). Do NOT rerun open-ended review passes until one happens to return clean — a later clean pass is sampling noise, not evidence of resolution.
3. **Maintain a manual finding ledger** (ephemeral working-state — session notes/scratch; you author it directly). One entry per finding, carrying: a stable local ID (F1, F2, …) that never changes once assigned; provenance (which reviewer/pass raised it, against which artifact/sha); the finding's evidence; and its current disposition. Reviewer output — including review-loop's notes and verdicts — is INPUT to this ledger; it is NOT the durable ledger and NOT the completion signal.
4. **Explicitly disposition EVERY finding before any fixing.** Exactly one disposition each:
   ...
   - `rejected_premise` — REQUIRES candidate-bound disproving evidence (a test, command output, or code path that contradicts the premise); "seems unlikely" is not a rejection;
   - `deferred` — REQUIRES a durable destination (a filed backlog ticket, cited by number);
   - `accepted_risk` — REQUIRES the applicable authority: operator authority whenever it changes a stated threat model, invariant, or acceptance scope;
   - `architecture_question` — goes to the operator/architecture conversation; never dispatched to a fixer.
5. **One batch fix.** Send ALL current `fix_now` findings to ONE fixer in ONE structured payload naming each finding ID with its evidence. Not one fixer per round; not fix-as-you-go.
6. **Deterministic verification AFTER the batch fix** — same gates as step 1, at the post-fix sha.
7. **Recheck exactly; bound the delta.** Recheck the KNOWN finding IDs exactly — does each specific mechanism still exist at the current sha — and run bounded discovery over ONLY the batch-fix delta (the fixer's diff). Not another open-ended pass.
8. **Resolution requires evidence.** A demonstrated finding becomes resolved only via resolution evidence — a test, command output, or inspection of the exact mechanism at the current sha. **Absence from a later reviewer's output is NEVER resolution.**
9. **New findings from the delta review** are recorded in the ledger and dispositioned like any other (step 4). They do NOT automatically begin another discovery/fix loop — running another cycle is a fresh, explicit decision.
10. **Incomplete review is not a clean result.** A selected reviewer that crashes, times out, or produces missing/malformed output leaves the review INCOMPLETE — retry that reviewer or record an authorized acceptance of the missing evidence. Never synthesize a pass or an empty finding set.
11. **Every existing deterministic and authority gate is unchanged**: the required CI merge gate (`test` AND `test-extended`), the acceptance-evidence closing gate, reviewed-tip identity/equality (FG-502/FG-514), candidate-identity and publication rules, branch protection, and the human-authority gates for governance/policy changes.

**Discovery transport.** `forge review-loop <ticket-id> --max-rounds 1 --route <resolved-route>` MAY be used as the transport for the single discovery pass (step 2) — its reviewer dispatch, workspace readiness, verification wiring, and tip-trust check all still earn their keep. Its output feeds the ledger; its stop reason is not the completion signal.

```bash
forge review-loop <ticket-id> --max-rounds 1 --route <resolved-route>
...
```bash
forge review-loop <ticket-id> --max-rounds 1 --route <resolved-route> --project <clone-path>
```

This interim evidence-led review is the **policy-derived default** for landed implementation work that changes code or durable behavior — not an operator preference. Do NOT ask the user to choose between it and a human PR review; run the sequence (FG-436). Ask before *skipping* review only for an explicit exception: docs-only, backlog-only, trivial metadata, emergency/unblock work, or when the user explicitly asked to skip automated review.

Rules:
- **Post-implementation ONLY.** The review sequence covers already-committed work — it is NOT for the initial implementation. You still own route resolution and the first implementation dispatch (for Forge-on-Forge, the first implementation you do directly, in a clone — see the `--project` note above), and you commit it before reviewing.
- **Present before you start the discovery pass:** ticket id, route key, commit range (or `--since`), the transport (review-loop `--max-rounds 1` or a targeted read-only red invoke), the reviewer role, the verification commands, the required CI checks (`test` and `test-extended` — `.github/workflows/ci.yml`), and that the pass is single-round discovery feeding the ledger. (`forge review-loop … --dry-run` prints most of this.)
- **`blocked_by_reviewer` / `needs_fix_max_rounds` on the single pass are EXPECTED discovery outcomes**, not stop-and-ask conditions by themselves — they mean the pass produced findings; ingest them into the ledger (step 3) and continue the sequence. Stop and ask the user whenever the work would need live spend, a credential, a live DB migration, a destructive operation, or a product/acceptance decision — the review sequence never auto-does any of those. (A routine fix/defer disposition on a finding is NOT one of these — apply the **Review-disposition & autonomy policy** above and act.)
...
- **Close the ticket only when the review sequence is COMPLETE**: every ledger finding settled with the evidence its disposition requires (rule 8 — absence from later reviewer output never settles anything), deterministic verification green at the final sha, and the reviewed tip trusted (below) — then close per **Before closing a backlog ticket**. A natural `closeable` (or a confirmed `closeout_guidance_only`) on the single discovery pass satisfies the model-review side directly. Any other legacy stop reason is NOT itself a close blocker once the evidence-led checks are satisfied — see **Legacy control-plane state** below for how that override is surfaced; it is never resolved by rerunning the loop.
...
- **Wait completion-driven — a launch-completion wake advances the transition; `ScheduleWakeup` is ONLY a lost-signal watchdog, never a job-duration timer (FG-563 / BD-9).** Sizing a wakeup "matched to how long the work takes" is a defect, not the wait path — the work signals its own completion, and no `sleep N` in Bash ever waits on launched work. The division of labor: **tmux (`forge launch run`) owns the work, `forge launch wait` observes its completion, `forge continue` claims and advances the transition, and `ScheduleWakeup` is only a fixed health-bound backstop for a lost wake.** For every long transition:
  1. **Launch + name the continuation.** Pick a stable `--continuation-id` for the logical next transition, then run the work under a durable owner: `forge launch run --require-control-toolchain [--name <n>] -- <command…>` — note the launch id. (`forge launch run` owns the work per FG-535; you only observe it.)
