...
| `lint-and-test` | Runs `nx lint <project>` and `nx test <project> --coverage`, then uploads coverage to Codecov. Also runs the V1 Cosmos export tool's real-SDK test tier (MG-49) and the MG-67 IoT device fixture sender's real-SDK test tier, each once per matrix leg, after `npm ci` — see [V1 Cosmos export tool](../../apps/infrastructure/README.md#v1-cosmos-export-tool-mg-48) and [Dev IoT device fixture](../../apps/infrastructure/README.md#dev-iot-device-fixture--device--iot-hub--route--cosmos-mg-67) | `api`, `web`, `mobile`, `api-interfaces` |
...
|------|-------|------------------------|--------------|
| **PR validation (credentialless)** | `ci.yml` → `validate-infrastructure` | **no identity, no environment** | `assert-credentialless.sh` → `terraform fmt -check -recursive` → `terraform init -backend=false -input=false -lockfile=readonly` → `terraform validate` → `terraform test` (root) → `tf-static-checks.sh` → `bootstrap.test.sh` → destroy-guard fixtures → V1 Cosmos export tool tests (MG-48, `run-tests.mjs` against a fake client — no network, no credentials) → dev IoT device fixture tests (MG-67, `iot-fixture/run-tests.mjs` against an injected fake reader and fake spawn — no network, no credentials) → per-module `terraform test` (`modules/functions`, `modules/iot-hub`, `modules/monitoring`) → OTel config validate. **No `terraform plan`, no backend, no Azure login** |
...
A concrete example: `cosmos_enable_free_tier` (MG-48) sets `free_tier_enabled` on the Cosmos account, an attribute Azure only accepts at account creation, so *any* change to it — in either environment — plans a **destroy** of `module.cosmos_db.azurerm_cosmosdb_account.main` before recreating it. The automatic `workflow_run` path carries no `authorized_changes` input at all, so it cannot pass this token and the circuit-breaker refuses that apply by design. Clearing it requires the branch-restricted `workflow_dispatch` recovery path with `delete:module.cosmos_db.azurerm_cosmosdb_account.main` in `authorized_changes`, reviewed and approved per the MG-23 procedure linked above.
