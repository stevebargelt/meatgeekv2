...
**2026-08-06 later.** MG-48 Phase 1+2 DONE except the free-tier apply. Deleted (all verified, archive at ~/meatgeek-v1-archive/ 84MB + cosmos-export/): APIM meatgeek (~$34.61), ACR acrmeatgeekbadsb2opzrqyk (~$5, image preserved), Event Hubs meetgeek (~$11), RG MeatGeek-IoT-Test-Devices (~$10-20), V1 Cosmos meatgeek (free-tier slot RELEASED) — its 9,886 docs (42 sessions/9,843 statuses/1 cook) exported and triple-verified first. Operator had already removed testhubmeatgeek (~$25). Azure did NOT delete anything during the disabled window — operator did. ~$85-115/mo reclaimed from ~$182.

**BLOCKED ON GITHUB ACTIONS MAJOR OUTAGE (ongoing 2026-08-06):** two runs cancelled mid-flight, jobs cancelled ~15min in. Nothing wrong with our code. Blocked: (a) MG-42 AC4's environment-scoped azure/login proof — the ONLY thing left before MG-42 closes; re-run CI on main once Actions is operational, then the dev apply provides the proof. (b) PR for branch feat/mg-48-cosmos-free-tier (tip 70b3c16, unmerged, deliberately not opened until CI can run).

**MG-48 free-tier apply — READ BEFORE MERGING.** `authorized_changes` is a workflow_dispatch-ONLY input; the automatic workflow_run apply path cannot pass it. So merging feat/mg-48-cosmos-free-tier makes the automatic apply plan a Cosmos destroy, the fail-closed guard REFUSES, and main's apply goes RED by design until someone runs the branch-restricted workflow_dispatch recovery path with token `delete:module.cosmos_db.azurerm_cosmosdb_account.main` and approves the required-reviewer gate. Merge and apply in one sitting, or expect a red window. Also check the plan's destroy list covers the account's CHILDREN (database, 5 containers, 2 sql_role_assignments) — they reference a deterministic account name so terraform may plan them unchanged even though Azure deletes them with the account.

Filed: MG-49 (cosmos-export --auth aad documented but @azure/identity not installed; V2 dev has disableLocalAuth=true so a key-only tool cannot export V2).
