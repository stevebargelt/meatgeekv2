...
`--device`, `--account`, `--database`, `--container`) are each validated against
the **actual Azure naming rule for that resource type** — an **allowlist**, so a
value is accepted only if it is a legal name of its own kind — and a rejection
names the **flag and the rule, never the value typed**, so no unvalidated argv
text reaches a log, the `az` argv, a document body or the evidence record. That
refuses connection strings and JWTs by construction (on their `/`, `;` and dots),
but **it is not a credential detector, and the runbook says so plainly**: a secret
shaped like a legal name — a 32-character device key pasted as `--device`, which
no naming rule can tell apart from a 32-character device id — is **not** screened
out. The guarantee is the narrower true one: the tool never accepts, holds or
...
| 1    | usage error — includes a refused credential-bearing or verbosity flag, a **name that fails its Azure resource-type rule** (the value is not echoed back), and an unusable or reused `--evidence-out` directory caught by the per-run **reservation before anything is sent** (`--evidence-out` is a directory; the tool names an immutable per-run file inside it — there is no `--overwrite`)                                                                                                                                                                                                                                                                                                                                                                                                     |
